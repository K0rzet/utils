...
def _xy_p_to_flat(xy: Optional[np.ndarray], p: Optional[np.ndarray]) -> Optional[List[float]]:
    ...
    for i in range(N):
        ...
        p_val = pp[i]
        out.extend([
            float(x_val) if x_val is not None else 0.0,
            float(y_val) if y_val is not None else 0.0,
            float(p_val) if p_val is not None else 0.0,
        ])
    return out
...
def _flat_to_xy_p(flat: Optional[List[float]]) -> Tuple[Optional[np.ndarray], Optional[np.ndarray]]:
    ...
    for i in range(N):
        xy[i, 0] = _safe_float(flat[3 * i + 0])
        xy[i, 1] = _safe_float(flat[3 * i + 1])
        p[i] = _safe_float(flat[3 * i + 2])
    return xy, p
